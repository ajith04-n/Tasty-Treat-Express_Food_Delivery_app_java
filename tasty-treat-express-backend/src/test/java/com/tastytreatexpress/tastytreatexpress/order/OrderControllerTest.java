...
public class OrderControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        menuItem = new MenuItem("Pizza", "Delicious pizza", 200.0, "Italian", "http://example.com/pizza.jpg", 2);
        menuItem.setId(1L);

        order = new Order();
        order.setOrderId(1L);
        order.setCustomer(
                new User("diwakar.allu.3435@gmail.com", "DiwakarAllu", "password", "123 Street", "1234567890"));
        order.setRestaurant(
                new Restaurant("Test Restaurant", "123 Restaurant Street", "Great food!", "test@example.com",
    ...
    public void testPlaceOrder_Success() throws Exception {
        Mockito.when(orderService.placeOrder(Mockito.anyLong(), Mockito.anyString(), Mockito.anyList(),
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/orders/placeOrder")
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.totalAmount").value(order.getTotalAmount()));
    }
